...
LAMODA_RETURNS_TOPIC_ID=
LAMODA_CANCELLATION_TIME=14:00
```
...
При приёмке бракованного возврата бот упоминает в теме руководителя бренда,
руководителя склада и операционщика. Адресаты определяются по активным
должностям сотрудников, а не по зашитым username.

При оприходовании возврата обязательна товарная этикетка Lamoda (`itemId`).
Паковую этикетку можно пропустить, если она не сохранилась на возврате. ЧЗ
также можно пропустить: для товара без исходного КИЗ возврат считается
немаркируемым и не создаёт операцию возврата в оборот; пропуск ЧЗ у ранее
маркированного товара отправляет приёмку на ручную сверку.

По воскресеньям, вторникам и четвергам в `LAMODA_CANCELLATION_TIME` бот
проверяет машины на следующий день. Если нет отгрузок или готовых возвратов,
он отправляет сообщение с PDF в личку активным руководителю склада,
руководителю бренда и операционщику. Для отгрузочной и возвратной машин
используются разные сообщения и PDF-шаблоны. Внешняя почта не используется.

### Несколько должностей сотрудника

В справочнике «Сотрудники» можно отметить несколько должностей одновременно.
Поле `role` сохраняется для совместимости со старыми данными, полный список
хранится в `roles`. Права сотрудника объединяются по всем выбранным должностям.

Стартовые назначения:

- Андрей Козырь — руководитель склада и администратор;
- Дмитрий Тарасов — сотрудник склада и операционщик.

### Массовое изменение расписания

Руководитель выбирает сотрудника и видит сразу семь дней недели. Галочка слева
добавляет или убирает смену, кнопка справа переключает время `10:00/15:00`.
Все изменения недели сохраняются одной кнопкой; дежурство автоматически
снимается с дня, в который сотруднику убрали смену.

### Отчёт руководителя склада

Складская часть (часы, задачи и KPI) и руководительская часть сохраняются и
отправляются разными сообщениями. Руководительский отчёт дополнительно уходит
активному руководителю бренда в личные сообщения и хранится отдельно от
ежедневного зарплатного отчёта.

Если у руководителя склада нет смены, в разделе расчёта ЗП доступна отдельная
кнопка `🧭 Руководительский отчет`. В мастере проверяются числовые объёмы,
оценка дня от 1 до 10, пустые и слишком длинные ответы; на каждом этапе можно
вернуться назад.
...
- ЗП и расписание в отдельные таблицы:
  ...
  `payroll_penalties`, `payroll_bonuses`, `payroll_kpi`, `payroll_periods`,
  `payroll_kpi_daily`, `schedule_archive`, `schedule_duties`,
  `payroll_additional_payments`, `schedule_exports`.
...
### Дополнительные начисления Trend Island

Руководитель бренда и администратор могут оформить руководителю склада недельное
начисление в разделе «Расчёт ЗП → Доп. начисления». Неделя всегда считается с
понедельника по воскресенье. Формула выплаты:

```text
количество поставок × 2 000 ₽ + 1 500 ₽ за неделю − штраф за ошибки
```

Недельная часть начисляется только при наличии хотя бы одной поставки. Штраф
за ошибки задаётся вручную и не может превышать начисление до штрафа. В расчётный
период запись попадает по воскресенью соответствующей недели. За одну неделю для
руководителя склада разрешена только одна запись Trend Island.

Каждый понедельник в 13:00 по Москве бот отправляет активным сотрудникам с ролями
`brand_manager` и `admin` личное напоминание, если запись за прошедшую неделю ещё
не оформлена. После создания или изменения записи руководитель склада получает
личный отчёт с периодом, количеством поставок, ошибками, штрафом и итоговой суммой.
...
Перед выгрузкой бот спрашивает, действуют ли сейчас скидки, а затем запрашивает название документа
«Вывод из оборота» в МойСклад. При наличии скидок для CSV и Excel используется тип цены
`Старая цена`, без скидок — `Цена продажи`. Для каждой позиции бот получает размерный артикул
из характеристики варианта `Артикул`, GTIN и коды маркировки. Если у варианта такой
характеристики нет, используется артикул самого варианта или родительской модели. Для CSV цена без НДС
...
`Товары легкой промышленности`; категория из МойСклад не используется. Значение столбца `Цвет`
извлекается из названия товара в справочнике Честного ЗНАКа, а не из МойСклад.

После ввода документа бот спрашивает, есть ли в поставке немаркируемая продукция. При ответе
`Да` руководитель выбирает товары из предопределенного справочника и указывает количество.
Немаркируемые товары добавляются только в Excel: наименование и размер берутся из локального
справочника, остальные реквизиты — из соответствующей позиции документа МойСклад. Для таких
строк столбец `Категория` остается пустым. На каждом шаге сценария доступна кнопка `Назад`.
...
`📚 Справочник Честного ЗНАКа` и не перезаписываются при перезапуске. Маркируемые товары
определяются заполненным GTIN; у немаркируемых товаров GTIN отсутствует.
