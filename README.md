...
## Автоматизация отчетов склада

В руководительском отчете бот предлагает значения из ежедневных отчетов за
выбранную дату: отправки, сток, возвраты и оприходование. Оприходование считается
как сумма количеств всех KPI упаковки; приемку руководитель вводит вручную.
Можно принять подсказку, ввести свой ответ или обновить расчет. Подсказка учитывает
текущий черновик складского отчета руководителя и показывает недостающие отчеты
сотрудников по расписанию.

Если отправок не было, бот предлагает выполненные складские задачи, выгруженные
в тему `SCHEDULE_REMINDER_TOPIC_ID`. Учитываются отметки выполнения в модуле задач.
При наличии отправок предлагается «Не актуально». Планы на завтра по-прежнему
собираются из задач автоматически.

После сохранения всех ежедневных отчетов сотрудников со сменой в расписании
(включая руководителя склада) бот отправляет общую сводку активным руководителям
бренда с указанным `telegram_user_id`. Сводка включает личные отчеты и суммы
количеств по каждому KPI. Сданные отчеты вне расписания также включаются с пометкой.
При пустом расписании автоматическая отправка выполняется только после отдельного
руководительского отчета и при наличии ежедневных отчетов сотрудников. Такой отчет
завершает ожидание руководителя и запускает сводку даже в его выходной; в KPI-итоги
входят только ежедневные отчеты сотрудников.

Изменение ежедневного отчета обновляет сводку. Длинный текст отправляется частями
без сокращения. Состояние доставки хранится в таблице PostgreSQL
`payroll_daily_summaries`, создаваемой при запуске бота. Неоконченные доставки
проверяются каждую минуту, в том числе после перезапуска; уже доставленные
неизменившиеся части повторно не отправляются. Проверяются также изменения
расписания для отслеживаемых сводок за сегодня и вчера. Исторические отчеты при
первом запуске автоматически не рассылаются.
...
В разделе «Разовые пропуска» бот собирает дату, ФИО посетителя и при
необходимости данные автомобиля, после чего формирует одностраничный PDF для
печати. Для грузового автомобиля дополнительно выбирается диапазон
грузоподъёмности. При отсутствии отчества в бланк подставляется `-`.

Данные постоянных посетителей и автомобилей можно сохранить как общий шаблон.
Дата в шаблоне не хранится и выбирается заново при каждом оформлении. Шаблоны
можно использовать, изменять, переименовывать и удалять.
