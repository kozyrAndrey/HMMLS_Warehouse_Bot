...
from schedule_excel import create_schedule_xlsx
...
async def schedule_day_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    selected = sorted(selected, key=lambda value: week_dates(context.user_data["schedule_week_start"]).index(__import__('schedule_config').parse_date(value)))
